...
2. 判断是公众号文章还是视频号视频
3. 自动调用 `weixin-to-feishu` skill 跑转录/抓取
4. 写入飞书云文档
5. 飞书私信推送文档链接给你
...
[wx-to-feishu worker]
   ...
   - SQLite 去重（避免重复处理）
       ↓
   ├─ 公众号文章 → weixin-to-feishu 抓正文 → lark-cli 写飞书
   └─ 视频号视频 → weixin-to-feishu transcribe → lark-cli 写飞书
       ↓
[飞书机器人] 推私信通知
```
...
cp .env.example .env  # 填飞书 open_id 和 weixin-to-feishu 路径
pip install -e .
...
| 公众号文章抓取失败 | 检查 `weixin-to-feishu` Playwright 的代理 / TLS 配置 |
| 视频号转录失败 | CDN 链接过期，重新转发即可 |
| worker 写飞书失败 | `lark-cli auth status` 看登录态 |

## 相关 skill

- `weixin-to-feishu`: 实际的"链接 → 飞书文档"实现，wx-to-feishu 在底层调用它
