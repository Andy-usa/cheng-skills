...
| 视频号视频 | `wxapp.tc.qq.com` | 方案 B：ffmpeg 抽音 → 火山引擎 BigASR（百炼 fallback）|
...
输出 JSON：`{"ok": true, "text": "...", "elapsed": 12.3, "duration": 168.0, "provider": "volc"}`

**脚本内部做了什么：**
1. `ffmpeg` 流式从 URL 抽音轨为 16kHz/mono/32kbps mp3（不保存视频本体；100MB 视频也只产生 ~1MB mp3）
2. base64 内联提交给火山引擎「录音文件识别大模型 标准版」(`volc.bigasr.auc`)
3. 异步轮询拿结果
4. 失败时自动 fallback 到百炼 qwen3-asr-flash（保留切片逻辑）

**为什么默认火山而非百炼：**
- 火山 BigASR 标准版**单次支持 ≤5h / ≤1GB 音频**，不需要切片，代码简单
- 火山中文识别准确率 + 标点 + ITN 实测优于 qwen3-asr-flash（尤其专有名词 / 方言）
- 异步任务模式，submit 秒返，poll 间隔 2s
- 状态码 `20000003`（无人声）也算成功，返回空字符串而不是抛错

**ASR Provider 切换：**
- 环境变量 `ASR_PROVIDER=auto`（默认）→ 先试火山，失败自动切百炼
- `ASR_PROVIDER=volc` → 仅火山
- `ASR_PROVIDER=dashscope` → 仅百炼

**依赖：**
- `ffmpeg`（`apt-get install -y ffmpeg` / `brew install ffmpeg`）
- 火山：`VOLC_ASR_APP_KEY` + `VOLC_ASR_ACCESS_KEY`
- 百炼（fallback）：`DASHSCOPE_API_KEY`
...
视频是一段口播稿，不是一篇结构化文章。**默认不加任何 `##`/`###` 小标题**——只用空行分段、只用排版手段做层次。

具体做法：
- 按语义切自然段，每段一个空行隔开。一段一个意思，**3–5 句话最舒服**
- 句子内部是连贯的口语流，**不要在句子中间硬加标题**
- 强调用 `**加粗**`，金句用 `> 引用块`，结构性的并列项可以用 `1.`/`2.`/`-` 的列表
- 篇章级别的话题切换（比如从"问题"到"解决方案"、从"现象"到"反思"）才用 `---` 分割线，**全文最多 1–2 处**
- **绝对不要** 把口播里"第一点、第二点、第三点"这种顺口的列举提升成 `## 第一件事`/`## 第二件事`——保持原文的"我下面要讲三点"的叙述感更好读

只有这两种情况可以用 `##`：
1. 视频本身就是结构化讲解（例如教程、SOP、列表式内容），且分段在原文中清晰可辨
2. 篇幅特别长（>10 分钟），不分章节难以扫读
...
| 火山 ASR 报 `Volc HTTP 401` | 检查 `VOLC_ASR_APP_KEY` / `VOLC_ASR_ACCESS_KEY` 是否填对；火山控制台「服务接口认证信息」处复核 |
| 火山 ASR 报 `Volc submit rejected` 状态码 4xxxxxxx | 4500000x 是参数问题；检查 mp3 是否为空 / 格式异常 |
| 火山返回空文本（status 20000003）| 这是"无人声内容"的合法状态，不是错误；视频本身就没人说话 |
| 走 fallback 时 `transcribe.py` 报 "audio is too long" | 火山失败、百炼接管时遇到 base64 上限；把 `CHUNK_SECONDS` 调小（如 120）后重试 |
...
| `VOLC_ASR_APP_KEY` + `VOLC_ASR_ACCESS_KEY` | [火山引擎语音控制台](https://console.volcengine.com/speech/service) → 录音文件识别大模型 → 服务接口认证信息 | 火山 BigASR 鉴权（默认） |
| `DASHSCOPE_API_KEY` | 阿里云百炼控制台 | qwen3-asr-flash 鉴权（fallback） |
| `ASR_PROVIDER` | `auto`（默认）/ `volc` / `dashscope` | 切换 ASR 提供商 |
